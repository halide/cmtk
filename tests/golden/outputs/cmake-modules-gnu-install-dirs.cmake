...
function(_GNUInstallDirs_cache_path var description)
  ...
  if (COMMAND _GNUInstallDirs_${var}_get_default)
    ...
    unset(last_default)
    if (
      DEFINED _GNUInstallDirs_LAST_CMAKE_INSTALL_PREFIX
    ...
    if (DEFINED CACHE{${cmake_install_var}} AND install_prefix_is_same)
      ...
      cmake_language(CALL _GNUInstallDirs_${var}_get_default default "${CMAKE_INSTALL_PREFIX}")
      # if the current value is the same as the cache value and the same as
      # the old default, reset the value to the new default
      if (
        ${cmake_install_var} STREQUAL "$CACHE{${cmake_install_var}}"
...
function(_GNUInstallDirs_get_system_type_for_install out_var)
  ...
  elseif (DEFINED ENV{CONDA_PREFIX})
    ...
    cmake_path(ABSOLUTE_PATH conda_prefix NORMALIZE)
    if (
      "${CMAKE_INSTALL_PREFIX}" STREQUAL conda_prefix
...
function(__GNUInstallDirs_default_in_usr out_var initial_value install_prefix)
  ...
  if (install_prefix STREQUAL "/")
    cmake_policy(
      GET
      CMP0193 cmp0193 PARENT_SCOPE  # undocumented, do not use outside of CMake
    )
...
function(_GNUInstallDirs_LIBDIR_get_default out_var install_prefix)
  ...
  if (NOT DEFINED CMAKE_SYSTEM_NAME OR NOT DEFINED CMAKE_SIZEOF_VOID_P)
    message(
      AUTHOR_WARNING "Unable to determine default CMAKE_INSTALL_LIBDIR directory because no target architecture is known. "
...
foreach (dir IN ITEMS SYSCONFDIR LOCALSTATEDIR)
...
function(_GNUInstallDirs_RUNSTATEDIR_get_default out_var install_prefix)
  ...
  if (_GNUInstallDirs_CMP0192 STREQUAL "NEW")
    ...
    if (install_prefix MATCHES "^/opt/" AND NOT install_prefix MATCHES "^/opt/homebrew/")
      string(REPLACE "${install_prefix}" "/run${install_prefix}" ${out_var}
...
_GNUInstallDirs_cache_path(BINDIR "User executables")
_GNUInstallDirs_cache_path(SBINDIR "System admin executables")
_GNUInstallDirs_cache_path(LIBEXECDIR "Program executables")
_GNUInstallDirs_cache_path(SYSCONFDIR "Read-only single-machine data")
_GNUInstallDirs_cache_path(SHAREDSTATEDIR "Modifiable architecture-independent data")
_GNUInstallDirs_cache_path(LOCALSTATEDIR "Modifiable single-machine data")
_GNUInstallDirs_cache_path(LIBDIR "Object code libraries")
_GNUInstallDirs_cache_path(INCLUDEDIR "C header files")
_GNUInstallDirs_cache_path(OLDINCLUDEDIR "C header files for non-gcc")
_GNUInstallDirs_cache_path(DATAROOTDIR "Read-only architecture-independent data root")
...
if (CMAKE_SYSTEM_NAME MATCHES "^(([^kF].*)?BSD|DragonFly)$")
  ...
  _GNUInstallDirs_cache_path(INFODIR "Info documentation")
else ()
  set(_GNUInstallDirs_INFODIR_DEFAULT "${CMAKE_INSTALL_DATAROOTDIR}/info")
  _GNUInstallDirs_cache_path_fallback(INFODIR "Info documentation (DATAROOTDIR/info)")
endif ()
...
if (
  ...
  _GNUInstallDirs_cache_path(MANDIR "Man documentation")
else ()
  set(_GNUInstallDirs_MANDIR_DEFAULT "${CMAKE_INSTALL_DATAROOTDIR}/man")
  _GNUInstallDirs_cache_path_fallback(MANDIR "Man documentation (DATAROOTDIR/man)")
endif ()
...
_GNUInstallDirs_cache_path_fallback(DATADIR "Read-only architecture-independent data (DATAROOTDIR)")
_GNUInstallDirs_cache_path_fallback(LOCALEDIR "Locale-dependent data (DATAROOTDIR/locale)")
_GNUInstallDirs_cache_path_fallback(DOCDIR "Documentation root (DATAROOTDIR/doc/PROJECT_NAME)")
_GNUInstallDirs_cache_path_fallback(RUNSTATEDIR "Run-time variable data (LOCALSTATEDIR/run)")
...
# Save for next run
set(_GNUInstallDirs_LAST_CMAKE_INSTALL_PREFIX "${CMAKE_INSTALL_PREFIX}"
...
function(GNUInstallDirs_get_absolute_install_dir absvar var)
  ...
  if (GGAID_extra_arg_count GREATER "0")
    ...
    set(GGAID_dir "${dir}")
    message(
      AUTHOR_WARNING "GNUInstallDirs_get_absolute_install_dir called without third argument. "
