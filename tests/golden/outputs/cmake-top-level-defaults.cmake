...
if (NOT CMake_TEST_EXTERNAL_CMAKE)
  if (CMAKE_SYSTEM_NAME STREQUAL "HP-UX")
    message(
      FATAL_ERROR "CMake no longer compiles on HP-UX.  See\n"
  ...
  set(UTILITIES
    BZIP2 CPPDAP CURL EXPAT FORM JSONCPP LIBARCHIVE LIBLZMA LIBRHASH LIBUV NGHTTP2 ZLIB ZSTD
  )
  foreach (util IN LISTS UTILITIES)
    if (NOT DEFINED CMAKE_USE_SYSTEM_LIBRARY_${util} AND DEFINED CMAKE_USE_SYSTEM_LIBRARIES)
  ...
  option(CMAKE_USE_SYSTEM_LIBARCHIVE
    "Use system-installed libarchive" "${CMAKE_USE_SYSTEM_LIBRARY_LIBARCHIVE}"
  )
  if (CMake_ENABLE_DEBUGGER)
    option(CMAKE_USE_SYSTEM_CPPDAP
      "Use system-installed cppdap" "${CMAKE_USE_SYSTEM_LIBRARY_CPPDAP}"
    )
  endif ()
  ...
  cmake_dependent_option(CMAKE_USE_SYSTEM_ZLIB
    "Use system-installed zlib" "${CMAKE_USE_SYSTEM_LIBRARY_ZLIB}"
    "NOT CMAKE_USE_SYSTEM_LIBARCHIVE;NOT CMAKE_USE_SYSTEM_CURL" ON
  )
  cmake_dependent_option(CMAKE_USE_SYSTEM_BZIP2
    "Use system-installed bzip2" "${CMAKE_USE_SYSTEM_LIBRARY_BZIP2}"
    "NOT CMAKE_USE_SYSTEM_LIBARCHIVE" ON
  )
  cmake_dependent_option(CMAKE_USE_SYSTEM_ZSTD
    "Use system-installed zstd" "${CMAKE_USE_SYSTEM_LIBRARY_ZSTD}" "NOT CMAKE_USE_SYSTEM_LIBARCHIVE"
    ON
  )
  cmake_dependent_option(CMAKE_USE_SYSTEM_LIBLZMA
    "Use system-installed liblzma" "${CMAKE_USE_SYSTEM_LIBRARY_LIBLZMA}"
    "NOT CMAKE_USE_SYSTEM_LIBARCHIVE" ON
  )
  cmake_dependent_option(CMAKE_USE_SYSTEM_NGHTTP2
    "Use system-installed nghttp2" "${CMAKE_USE_SYSTEM_LIBRARY_NGHTTP2}" "NOT CMAKE_USE_SYSTEM_CURL"
    ON
  )
  option(CMAKE_USE_SYSTEM_FORM "Use system-installed libform" "${CMAKE_USE_SYSTEM_LIBRARY_FORM}")
  cmake_dependent_option(CMAKE_USE_SYSTEM_JSONCPP
    "Use system-installed jsoncpp" "${CMAKE_USE_SYSTEM_LIBRARY_JSONCPP}"
    "NOT CMAKE_USE_SYSTEM_CPPDAP" ON
  )
  option(CMAKE_USE_SYSTEM_LIBRHASH
    "Use system-installed librhash" "${CMAKE_USE_SYSTEM_LIBRARY_LIBRHASH}"
  )
...
if (NOT CMake_TEST_EXTERNAL_CMAKE)
  ...
  if (CMake_ENABLE_INSTRUMENTATION)
    cmake_instrumentation(API_VERSION 1 DATA_VERSION 1 OPTIONS cdashSubmit cdashVerbose trace)
  endif ()
endif ()
...
option(CMake_INSTALL_DEPENDENCIES "Whether to install 3rd-party runtime dependencies" OFF)
mark_as_advanced(CMake_INSTALL_DEPENDENCIES)

# option to build reference for CMake developers
option(CMake_BUILD_DEVELOPER_REFERENCE "Build CMake Developer Reference" OFF)
...
if (NOT CMake_TEST_EXTERNAL_CMAKE)
  if (NOT DEFINED CMake_ENABLE_DEBUGGER)
    ...
    if (
      CMAKE_SYSTEM_NAME MATCHES "Windows|Darwin|Linux|BSD|DragonFly|CYGWIN|MSYS"
      AND NOT (CMAKE_CXX_COMPILER_ID STREQUAL "MSVC"
        AND CMAKE_CXX_COMPILER_VERSION VERSION_LESS 19.16)
      AND NOT (CMAKE_CXX_COMPILER_ID STREQUAL "XLClang"
        AND CMAKE_CXX_COMPILER_VERSION VERSION_LESS 16.1)
      AND NOT (CMAKE_CXX_COMPILER_ID STREQUAL "LCC"
        AND CMAKE_CXX_COMPILER_VERSION VERSION_LESS 1.23)
    )
...
if (CMake_RUN_CLANG_TIDY)
  ...
  if (CMake_USE_CLANG_TIDY_MODULE)
    find_library(
      CMake_CLANG_TIDY_MODULE
      NAMES cmake-clang-tidy-module
      DOC "Location of the clang-tidy module"
    )
    if (NOT CMake_CLANG_TIDY_MODULE)
      message(
        FATAL_ERROR "CMake_USE_CLANG_TIDY_MODULE is ON but cmake-clang-tidy-module is not found!"
      )
    endif ()
...
if (CMake_RUN_IWYU)
  ...
  if (CMake_IWYU_VERBOSE)
    list(APPEND CMAKE_CXX_INCLUDE_WHAT_YOU_USE -Xiwyu -v7)
  endif ()
...
if (CMake_RUN_PVS_STUDIO)
  ...
  set(CMAKE_CXX_PVS_STUDIO
    "${PVS_STUDIO_COMMAND}" analyze -a GA --apply-pvs-configs --project-root "${CMAKE_SOURCE_DIR}"
  )
endif ()
...
if (NOT CMake_TEST_EXTERNAL_CMAKE)
  if (CMAKE_CXX_PLATFORM_ID MATCHES "OpenBSD")
    ...
    string(REGEX REPLACE "([0-9])\\.([0-9])(\\.[0-9])?" "\\1\\2" _GXX_VERSION_SHORT ${_GXX_VERSION})
    if (_GXX_VERSION_SHORT EQUAL 33)
      message(
        FATAL_ERROR "GXX 3.3 on OpenBSD is known to cause CPack to Crash.\n"
...
set_property(DIRECTORY APPEND
  PROPERTY TEST_INCLUDE_FILES "${CMake_BINARY_DIR}/Tests/EnforceConfig.cmake"
)
...
if (NOT CMake_TEST_EXTERNAL_CMAKE)
  ...
  set(LIBRARY_OUTPUT_PATH "" CACHE INTERNAL "Where to put the libraries for CMake")
...
if (NOT CMake_TEST_EXTERNAL_CMAKE)
  ...
  if (UNIX)
    ...
    set(CMAKE_INSTALL_RPATH_USE_LINK_PATH ON
      CACHE BOOL "Install with RPATH set to find custom-built libraries."
    )
    set(CMAKE_BUILD_WITH_INSTALL_RPATH ON
      CACHE BOOL "Build with RPATH set to match install-tree RPATH."
    )
  ...
  if (NOT CMake_VERSION_IS_RELEASE)
    if (
      (CMAKE_C_COMPILER_ID STREQUAL "GNU"
        AND NOT "${CMAKE_C_COMPILER_VERSION}" VERSION_LESS 4.2)
      OR (CMAKE_C_COMPILER_ID STREQUAL "Clang"
        AND NOT "${CMAKE_C_COMPILER_VERSION}" VERSION_LESS 3.0
        AND NOT "x${CMAKE_C_SIMULATE_ID}" STREQUAL "xMSVC")
      OR CMAKE_C_COMPILER_ID STREQUAL "AppleClang"
      OR CMAKE_C_COMPILER_ID STREQUAL "LCC"
    )
      set(C_FLAGS_LIST
        -Wcast-align -Wchar-subscripts -Wall -W -Wpointer-arith -Wwrite-strings -Wformat-security
        -Wmissing-format-attribute -fno-common -Wundef -Werror=implicit-function-declaration
        -Wstrict-prototypes
      )
      set(CXX_FLAGS_LIST
        -Wnon-virtual-dtor -Wcast-align -Wchar-subscripts -Wall -W -Wshadow -Wpointer-arith
        -Wformat-security -Wundef
      )
      if (
        ...
        list(APPEND CXX_FLAGS_LIST -Wundefined-func-template)
      endif ()
...
if (BUILD_TESTING)
  add_test(
    SystemInformationNew "${CMAKE_CMAKE_COMMAND}" --system-information -G "${CMAKE_GENERATOR}"
  )
endif ()
