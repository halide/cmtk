...
block (SCOPE_FOR POLICIES)
  ...
  function(__FetchContent_declareDetails contentName)
    ...
    elseif (
      "${FETCHCONTENT_TRY_FIND_PACKAGE_MODE}" STREQUAL "OPT_IN"
    ...
    cmake_language(EVAL
      CODE "set_property(GLOBAL PROPERTY ${savedDetailsPropertyName} ${__cmdArgs})"
    )
    ...
    if (__tryFindPackage AND __tryFindPackageAllowed)
      ...
      cmake_language(EVAL CODE "set_property(GLOBAL PROPERTY ${propertyName} ${__findPackageArgs})")
    endif ()
  ...
  function(FetchContent_Declare contentName)
    ...
    if (index_OVERRIDE_FIND_PACKAGE GREATER_EQUAL 0 AND index_FIND_PACKAGE_ARGS GREATER_EQUAL 0)
      message(
        FATAL_ERROR "Cannot specify both OVERRIDE_FIND_PACKAGE and FIND_PACKAGE_ARGS "
    ...
    if (ARG_GIT_REPOSITORY)
      ...
      if (NOT alreadyDefined)
        cmake_policy(
          GET
          CMP0150 cmp0150 PARENT_SCOPE  # undocumented, do not use outside of CMake
        )
        _ep_resolve_git_remote(
          _resolved_git_repository "${ARG_GIT_REPOSITORY}" "${cmp0150}" "${FETCHCONTENT_BASE_DIR}"
        )
    ...
    if (NOT ARG_DOWNLOAD_NO_EXTRACT AND NOT DEFINED ARG_DOWNLOAD_EXTRACT_TIMESTAMP)
      ...
      if (urlIndex GREATER_EQUAL 0)
        ...
        if (urlIndex GREATER_EQUAL numArgs)
          message(FATAL_ERROR "URL keyword needs to be followed by at least one URL")
        endif ()
        ...
        if (NOT IS_DIRECTORY "${firstUrl}")
          cmake_policy(
            GET
            CMP0135 _FETCHCONTENT_CMP0135 PARENT_SCOPE  # undocumented, do not use outside of CMake
          )
          if (_FETCHCONTENT_CMP0135 STREQUAL "")
            message(
              AUTHOR_WARNING "The DOWNLOAD_EXTRACT_TIMESTAMP option was not given and policy "
    ...
    cmake_policy(
      GET
      CMP0168 cmp0168 PARENT_SCOPE  # undocumented, do not use outside of CMake
    )
    ...
    endforeach ()
    cmake_language(EVAL CODE "__FetchContent_declareDetails(${contentNameLower} ${__argsQuoted})")
  ...
  function(__FetchContent_populateDirect)
    ...
    set(args_to_parse
      "${ARG_UNPARSED_ARGUMENTS}" SOURCE_DIR "${ARG_SOURCE_DIR}" BINARY_DIR "${ARG_BINARY_DIR}"
    )
  ...
  function(__FetchContent_doStepDirect)
    set(noValueOptions)
    set(singleValueOptions SCRIPT_FILE STAMP_FILE)
    set(multiValueOptions DEPENDS)
    cmake_parse_arguments(
      PARSE_ARGV 0 arg "${noValueOptions}"
      "${singleValueOptions}" "${multiValueOptions}"
    )
    ...
    if (NOT EXISTS ${arg_STAMP_FILE})
      ...
      foreach (dep_file IN LISTS arg_DEPENDS arg_SCRIPT_FILE)
        if (
          NOT EXISTS "${arg_STAMP_FILE}"
          OR NOT EXISTS "${dep_file}"
          OR NOT "${arg_STAMP_FILE}" IS_NEWER_THAN "${dep_file}"
        )
          set(do_step YES)
          break()
        endif ()
...
"
    ...
    if (ARG_QUIET)
      set(outputOptions OUTPUT_VARIABLE capturedOutput ERROR_VARIABLE capturedOutput)
    else ()
      set(capturedOutput)
      set(outputOptions)
...
"
    ...
    configure_file(
      ...
      "${ARG_SUBBUILD_DIR}/CMakeLists.txt" @ONLY
    )
    execute_process(
  ...
  function(FetchContent_Populate contentName)
    ...
    if (ARGC EQUAL 1)
      cmake_policy(
        GET
        CMP0169 cmp0169 PARENT_SCOPE  # undocumented, do not use outside of CMake
      )
    ...
    cmake_policy(
      GET
      CMP0170 cmp0170 PARENT_SCOPE  # undocumented, do not use outside of CMake
    )
  ...
  function(__FetchContent_Populate contentName cmp0170)
    ...
    if (ARGN)
      ...
      __FetchContent_doPopulation(
        ...
        # Could override any of the above ..._DIR variables
        BINARY_DIR "${CMAKE_CURRENT_BINARY_DIR}/${contentNameLower}-build" ${ARGN}
      )
    ...
    if (${contentNameLower}_POPULATED)
      if ("${${contentNameLower}_SOURCE_DIR}" STREQUAL "")
        message(
          FATAL_ERROR "Content ${contentName} already populated by find_package() or a "
          "dependency provider"
        )
      else ()
        message(
          FATAL_ERROR "Content ${contentName} already populated in ${${contentNameLower}_SOURCE_DIR}"
        )
      endif ()
    endif ()
    ...
    set(FETCHCONTENT_SOURCE_DIR_${contentNameUpper} "${FETCHCONTENT_SOURCE_DIR_${contentNameUpper}}"
    ...
    if (FETCHCONTENT_SOURCE_DIR_${contentNameUpper})
      ...
      if (NOT IS_ABSOLUTE "${FETCHCONTENT_SOURCE_DIR_${contentNameUpper}}")
        ...
        message(
          WARNING "Relative source directory specified. This is not safe, "
      ...
      elseif (NOT EXISTS "${FETCHCONTENT_SOURCE_DIR_${contentNameUpper}}")
        message(
          FATAL_ERROR "Manually specified source directory is missing:\n"
          "  FETCHCONTENT_SOURCE_DIR_${contentNameUpper} --> ${FETCHCONTENT_SOURCE_DIR_${contentNameUpper}}"
        )
      endif ()
    ...
    elseif (FETCHCONTENT_FULLY_DISCONNECTED)
      ...
      if (NOT IS_ABSOLUTE "${${contentNameLower}_SOURCE_DIR}")
        message(
          WARNING "Relative source directory specified. This is not safe, as it depends "
  ...
  function(__FetchContent_setupFindPackageRedirection contentName)
    ...
    if (NOT EXISTS "${configFilePrefix1}.cmake" AND NOT EXISTS "${configFilePrefix2}.cmake")
      configure_file(${inFileDir}/package-config.cmake.in "${configFilePrefix2}.cmake" @ONLY)
    endif ()
    if (
      NOT EXISTS "${configFilePrefix1}Version.cmake"
      AND NOT EXISTS "${configFilePrefix2}-version.cmake"
    )
      configure_file(
        ${inFileDir}/package-config-version.cmake.in "${configFilePrefix2}-version.cmake" @ONLY
      )
    endif ()
  ...
  macro(FetchContent_MakeAvailable)
    ...
    foreach (__cmake_contentName IN ITEMS ${ARGV})
      ...
      if ("${FETCHCONTENT_SOURCE_DIR_${__cmake_contentNameUpper}}" STREQUAL "")
        ...
        if (
          NOT "${__cmake_providerCommand}" STREQUAL ""
          AND NOT DEFINED __cmake_fcProvider_${__cmake_contentNameLower}
        )
          message(
            VERBOSE "Trying FETCHCONTENT_MAKEAVAILABLE_SERIAL dependency provider for "
            "${__cmake_contentName}"
          )
          ...
          set(__cmake_providerArgs "FETCHCONTENT_MAKEAVAILABLE_SERIAL" "${__cmake_contentName}")
          ...
          # need preserved.
          list(APPEND __cmake_fcCurrentVarsStack ${__cmake_contentName} ${__cmake_contentNameLower})
        ...
        if (__cmake_haveFpArgs)
          ...
          list(APPEND __cmake_fcCurrentNameStack ${__cmake_contentName} ${__cmake_contentNameLower})
          ...
          list(POP_BACK __cmake_fcCurrentNameStack __cmake_contentNameLower __cmake_contentName)
          unset(__cmake_fpArgs)
      ...
      if (NOT ${__cmake_contentNameLower}_POPULATED)
        cmake_policy(
          GET
          CMP0170 __cmake_fc_cmp0170 PARENT_SCOPE  # undocumented, do not use outside of CMake
        )
        ...
        cmake_parse_arguments(
          __cmake_arg "EXCLUDE_FROM_ALL;SYSTEM" "SOURCE_SUBDIR" "" ${__cmake_contentDetails}
        )
        ...
        if (EXISTS ${__cmake_srcdir}/CMakeLists.txt)
          ...
          set(__cmake_add_subdirectory_args
            ${__cmake_srcdir} ${${__cmake_contentNameLower}_BINARY_DIR}
          )
    ...
    set(CMAKE_VERIFY_PRIVATE_HEADER_SETS ${__cmake_original_verify_private_setting})
    set(CMAKE_VERIFY_INTERFACE_HEADER_SETS ${__cmake_original_verify_interface_setting})
